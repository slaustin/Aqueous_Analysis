...
__global__ void countagua_2 (int *a,int *b,int *c,int na,int nb,float *xx,float *yy,float *zz,int *convers,int natim,int x_min,int y_min,int z_min,int x_max,int y_max,int z_max,int x_side,int y_side,int z_side,float *space,int num_points,int num_protons,float *atom_radius) {

        int k=threadIdx.x + blockDim.x * blockIdx.x;
        int r=blockIdx.x;
        int kk,j,jj,q,xxx,yyy,zzz,rx,ry,rz;
        long int n_of_grid;
        float curr_dist,dist_a,radius,dimensionality;
							...
							if(dist_a < 2.45){
								...
								if ((xxx >= x_min)&&(yyy >= y_min)&&(zzz >= z_min)&&(xxx <= x_max)&&(yyy <= y_max)&&(zzz <= z_max)){
									...
									if ((rx >= x_min)&&(ry >= y_min)&&(rz >= z_min)&&(rx <= x_max)&&(ry <= y_max)&&(rz <= z_max)){
										...
										if((n_of_grid >= 0)&&(n_of_grid < num_points)){
											radius=powf(((atom_radius[kk])/2.0),2.0);
                                                                                        dimensionality=(-3.0/2.0);
                                                                                        space[n_of_grid]+=powf((2.0*3.14*radius),dimensionality)*(expf((-1.0*dist_a*dist_a)/(2.0*radius)));}
									}
									}
									}
									}
...
extern "C" void water_wrapper_3_(int *frame_count, double *x,double *y,double *z,int *min_x,int *min_y,int *min_z,int *max_x, int *max_y, int *max_z,int *sidex,int *sidey,int *sidez,double *grid_array,int *natoma,int *lista,int *natomb,int *listb,int *conversion,int *natim, int *prot_num, int *curr_count,double *radius_array)
{//main
...
int *local_lista, *local_listb, *local_listc, *local_conversion;
float *local_x, *local_y, *local_z, *local_space, *local_space2;
int *dev_lista, *dev_listb, *dev_listc, *dev_conversion;
float *dev_x,*dev_y,*dev_z,*dev_space;
float *local_radius;
float *dev_radius;
...
local_space=(float *)malloc(num_points*sizeof(float));
if(local_space == NULL){
   printf("Error: %s:%d, ", __FILE__, __LINE__);
   exit(1);}

local_space2=(float *)malloc(num_points2*sizeof(float));
...
memset(local_space,0,num_points*sizeof(float));
memset(local_space2,0,num_points2*sizeof(float));
...
CHECK (cudaMalloc((void **) &dev_space, num_points*sizeof(float)) );
...
CHECK (cudaMemcpy(dev_space, local_space, num_points*sizeof(float), cudaMemcpyHostToDevice) );
...
countagua_2<<<blocks,threads>>>(dev_lista,dev_listb,dev_listc,num_atom_a,num_atom_b,dev_x,dev_y,dev_z,dev_conversion,n_atim,minn_x,minn_y,minn_z,maxx_x,maxx_y,maxx_z,x_side,y_side,z_side,dev_space,num_points,num_protons,dev_radius);

CHECK (cudaMemcpy(local_space, dev_space, num_points*sizeof(float), cudaMemcpyDeviceToHost) );
