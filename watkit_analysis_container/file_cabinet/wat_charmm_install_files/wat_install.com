...
#  echo "          i8  to request 64 bit integers"
  ...
  echo "      DOMDEC  compile with DOMDEC support"
  echo "  DOMDEC_GPU  compile with DOMDEC GPU support (switches on DOMDEC)"
...
#SAANA
# Modify Makefile_template for SAANA (=add nvcc compiler)
  if ( $saana == 1 ) then
    echo "SAANA CUDA Enabled"
    set nvcc_bin = `which nvcc`
    if ( $status == 0 ) then
        setenv CUDAC_ROOT $nvcc_bin:h:h
    else
...
switch ($chm_host)
  case gnu:
    ...
    if ($pgf95 == 1) echo "PGI" >> $chmbuild/pref$$.dat
...
if ( $stopmark == 2 ) then
  echo " "
  echo " install.com> Installation STOPs after creating "
  echo "              the default pref.dat and"
  echo "              the preprocessor prefx_$chm_host."
  echo " "
...
if ($domdec == 1) then
  foreach f ($chmsrc/domdec/*.inc)
    set new_domdec_include = $f:t:r
    /bin/cp "$f" "$chmbuild/$new_domdec_include.inc"
  end
endif
...
set go_make = "$go_make -f energy.mk -f gamint.mk -f gamus.mk -f gukint.mk"
...
set go_make = "$go_make -f sa_analysis.mk"
if ( $?nolog ) then
   $go_make 
   set rc = $status
else
